...
class TaskScheduler:
    ...
    async def dequeue(self, queue: str = "default", timeout: float = 1.0) -> Optional[Dict]:
        ...
        for tid in expired:
            ...
            if task:
                # Check if task already reached terminal state
                ...
                if current_state in (TaskState.COMPLETED, TaskState.FAILED, TaskState.CANCELLED):
                    ...
                    continue
                # Only enqueue if not already in flight or terminal
                if queue not in self._queues:
                    self._queues[queue] = PriorityQueue()
    ...
    def fail(self, task_id: str, queue: str = "default") -> bool:
        ...
        """
        current_state = self._task_states.get(task_id)
        # Guard: reject if already terminal
        if current_state in (TaskState.COMPLETED, TaskState.FAILED, TaskState.CANCELLED):
            return False
    ...
    def cancel(self, task_id: str) -> bool:
        ...
        for q in self._queues.values():
            # Can't directly remove, but mark as cancelled
            pass
